...
def create_ipdf_overlay(options_df, current_price):
    ...
    ax.set_caption(f'For {TICKER}', fontsize=14, fontweight='bold')
...
def create_mode_and_quartiles(options_df, current_price):
    ...
    for exp in expirations[:12]:
        ...
        if len(atm_option) > 0:
            ...
            exp_labels.append(f"{exp.strftime('%Y-%M')}")
            modes.append(mode)
            q25.append(q25_val)
            q75.append(q75_val)
            q5.append(q5_val)
...
time.sleep(2)
r = 0.04
# interest.history(period="1d")["Close"].iloc[-1]/100  # assume risk-free rate ~5% (you could pull ^IRX)
