...
def create_3d_ipdf_surface(options_df, current_price, ticker = "IDK"):
    """
...
create_3d_ipdf_surface(options_df, current_price, ticker="AAPL")
```
...
plt.clf()
fig, ax = plt.subplots(figsize=(12, 7))
ax.bar(range(len(options_df['expiration'].unique())), height = skew_df, color = "#ff7f0e")
ax.set_xticks(range(len(options_df['expiration'].unique())))
ax.set_xticklabels(exp_labels, rotation=45, ha='right')
plt.tight_layout()
```
...
plt.clf()
fig, ax = plt.subplots(figsize=(12, 7))
ax.bar(range(len(options_df['expiration'].unique())), height = skew_df)
ax.set_xticks(range(len(options_df['expiration'].unique())))
ax.set_xticklabels(exp_labels, rotation=45, ha='right')
plt.tight_layout()
```
...
# X, Y = np.meshgrid(price_ranges, np.repeat(np.arange(1, 20), 200))
# fig = plt.figure()
# ax = plt.axes(projection="3d")
# ax.plot_wireframe(X, Y, np.array(pdfs_matrix), color = "blue") #X, Y, Z
# plt.show()
...
```{python}
#| label: Risk_reversal
def calculate_risk_reversal(group, put_moneyness=0.90, call_moneyness=1.10):
    """
    ...
    try:
        ...
        puts = group[group['type'] == 'put'].drop_duplicates('moneyness')
        calls = group[group['type'] == 'call'].drop_duplicates('moneyness')
        
        if len(puts) < 2 or len(calls) < 2:
            return np.nan
        
        # Interpolate
        f_put = interp1d(puts['moneyness'], puts['impliedVolatility'], 
                        kind='linear', fill_value='extrapolate')
        f_call = interp1d(calls['moneyness'], calls['impliedVolatility'],
                         kind='linear', fill_value='extrapolate')
        
        iv_put = f_put(put_moneyness)
        iv_call = f_call(call_moneyness)
...
# Calculate
skew_timeseries['risk_reversal'] = np.array(df_analysis.groupby('expiration').apply(calculate_risk_reversal, include_groups=False))

```

```{python}
#| label: Polynomial_Skew
def calculate_polynomial_skew(group):
    """
    Fit: IV = a + b*(K/S - 1) + c*(K/S - 1)^2
    Skew coefficient = b (linear term)
    """
    if len(group) < 10:
        return np.nan
    
    # Calculate relative strike (K/S - 1)
    group = group.dropna(subset=['moneyness', 'impliedVolatility'])
    X = group['moneyness'] - 1.0
    y = group['impliedVolatility']
    
    # Fit polynomial
    coeffs = np.polyfit(X, y, deg=2)
    
    # Return linear coefficient (skew)
    return coeffs[1]

skew_timeseries['poly_skew'] = np.array(df_analysis.groupby('expiration').apply(calculate_polynomial_skew, include_groups=False))
skew_timeseries
```
...
# Test all metrics
metrics_to_test = ['atm_skew', 'risk_reversal', 'poly_skew']
test_results = []
